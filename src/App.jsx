...
const pricing = [
  {
    name: "Пилот",
    audience:
      "Для малого бизнеса, который хочет за 1 неделю проверить работу сценария на реальных клиентах.",
    price: "6 990 ₽ / неделя",
    cta: CTA_PRIMARY,
    points: [
      "7 дней теста на входящих лидах",
      "Базовая квалификация и повторный контакт",
      "Отчет по итогам недели: где рост и где потери",
      "План перехода на полноценное подключение",
    ],
  },
  {
    name: "Подключение",
    audience: "Для команды, которой нужен стабильный входящий контур и путь до оплаты без лишних модулей.",
    price: "49 990 ₽ / месяц",
    cta: CTA_PRIMARY,
    popular: true,
    points: [
      "Подключение канала, CRM и кабинета контроля",
      "Карта утечек и контроль этапов до оплаты",
      "Еженедельная оптимизация конверсии",
      "Звонки с ai в роли оператора и передачей менеджеру",
    ],
  },
  {
    name: "Сопровождение",
    audience: "Для компаний, которым нужен полный контур роста: входящий спрос, поиск новых клиентов и звонки.",
    price: "89 000 ₽ / месяц",
    cta: CTA_PRIMARY,
    dark: true,
    points: [
      "Все из пакета «Подключение»",
      "Поиск и приоритизация новых клиентов",
      "Голосовые сценарии с автопередачей менеджеру",
      "Постоянные обновления и рост по данным",
    ],
  },
];
      ...
      <main className="shell">
        ...
        <section className="section reveal" id="ceny">
          ...
          <p className="sectionSubtitle">
            Пакеты сделаны под малый и средний бизнес: стартуем с недельного пилота и подключаем полный контур поэтапно.
          </p>
            ...
            {pricing.map((plan) => (
              <article className={`priceCard ${plan.dark ? "dark" : ""}`} key={plan.name}>
                <div className="priceHead">
                  <h3>{plan.name}</h3>
                  {plan.popular ? <span className="priceBadge">Самый частый выбор</span> : null}
                </div>
